...
!      real(8), contiguous, dimension(:,:) :: tempc3a ! tempc3a(mdittal,nclass)
      ...
      iquotient1=(mditt-ittst+1)/iblout
      iquotient2=(k3-k1+1)/iblin
      iremainder1=mod(mditt,iblout)
      iremainder2=mod(k3-k2,iblin)

         if (mditt < ittbl) then

            do jj=1,iquotient1*iblout,iblout
               do ii=1,iquotient2*iblin,iblin
        ...
        if(iremainder1 > 0 .and. iremainder2>0) then
               do itt=ittst,mditt
                     tempc2=0.0
                    do ks=iquotient2*iblin+k1,k3 
                        tempc2 = tempc2 + adx(ks)*vq(ks,itt)
                     end do
                     tempc3a(itt,ica) = tempc3a(itt,ica) + tempc2
                  end do

               do itt=iquotient1*iblout+ittst,mditt
                    tempc2=0.0
                    do ks=k1,iquotient2*iblin+k1-1
                        tempc2 = tempc2 + adx(ks)*vq(ks,itt)
                     end do
                     tempc3a(itt,ica) = tempc3a(itt,ica) + tempc2
                  end do
