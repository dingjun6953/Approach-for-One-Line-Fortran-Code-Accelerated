...
!                                                 ! tempc3a(mdittal,nclass)
      ...
      real(8) tempc3a(mdittal,nclass)
         ...
         subroutine  t3
     +             (A, B, iMAX1, iMAX2, nthrds,
     +              blocksize1,blocksize2,ittbl,mditt )
      !real(8) A(iMAX,iMAX)
      !real(8) B(iMAX,iMAX)
...
!      kount_max = 2000000
     ...
     + nthrds,ivlenb,iblin, iblout
      ...
      allocate ( A(iMAX1,iMAX2*nthrds), stat = ierr )
      allocate ( B(iMAX1,iMAX2*nthrds), stat = ierr )
...
c$omp    do schedule(runtime)
         do ica = 1, nclass
            call t3(A,B,iMAX1,iMAX2,ica,iblout,iblin, ittbl, mditt) 
         enddo
c$omp    end parallel
      enddo
...
!!         !DIR$ VECTOR ALIGNED
      ...
      subroutine t3(A,B,iMAX1, iMAX2, nthrds,blocksize1 ,
     +    blocksize2,ittbl,mditt )
      ...
      integer iMAX2, blocksize2,nthrds
      integer i,j, ii, jj
      ...
      integer temp1
       ...
       temp1=iMAX2*(nthrds-1)

         if (mditt < ittbl) then
         !print *, 'enter blockloop branch....'
         do i=1,iquotient1*blocksize1,blocksize1
         do j=temp1+1,temp1+iquotient2*blocksize2,blocksize2
        ...
        if(iremainder1 > 0 .and. iremainder2>0) then 
         do i =1, iMAX1 
          do j = temp1+1+iquotient2*blocksize2,iMAX2*nthrds 
                 A(i,j) = A(i,j) + B(i,j)
          enddo 
          enddo
          do i =iquotient1*blocksize1+1, iMAX1 
                !DIR$ VECTOR ALIGNED
          do j =temp1+1, temp1+iquotient2*blocksize2 
                    A(i,j) = A(i,j) + B(i,j)
                enddo 
           enddo 
        endif
       ...
       do i =1, iMAX1 
                !DIR$ VECTOR ALIGNED
       do j = temp1+iquotient2*blocksize2+1,iMAX2*nthrds
                    A(i,j) = A(i,j) + B(i,j)
                enddo 
           enddo 
       endif
