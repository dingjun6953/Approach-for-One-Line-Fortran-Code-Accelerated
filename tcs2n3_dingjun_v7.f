...
!                                                 ! tempc3a(mdittal,nclass)
      ...
      subroutine t3(A, B, iMAX, blocksize, ittbl, mditt) 
      !real(8) A(iMAX,iMAX)G
      !real(8) B(iMAX,iMAX)
...
!      kount_max = 2000000
     ...
     + ivlenb,iblin, iblout
      ...
      allocate ( A(iMAX,iMAX), stat = ierr )
      allocate ( B(iMAX,iMAX), stat = ierr )
...
c$omp    do schedule(runtime)
         do ica = 1, nclass
            !call t3(A,B,iMAX, iblin, ittbl, mditt) 
...
!bp                     do 405 ks = ksts(icatis(ica)+1)+1, kstscf(ica)
      ...
      integer iMAX, blocksize,mditt, ittbl
      integer i,j, ii, jj
      !real(8) A(iMAX,iMAX)
      !real(8) B(iMAx,iMAx)
         ...
         !print *, 'enter blockloop branch....'
            do i=1,iMAX,blocksize
                do j=1,iMAX,blocksize
                    do ii=i,i+blocksize-1
     ...
     +    ittbl, mditt,iblin,iblout)
      ...
      real(8) f
         ...
         do jj=1,kount_max/iblout+1
              do ii = 1,kount_max/iblin+1
                  do j = (jj-1)*iblout+1, min(jj*iblout, kount_max)
                      f = 0
                      do i = (ii-1)*iblin+1, min(ii*iblin,kount_max)
